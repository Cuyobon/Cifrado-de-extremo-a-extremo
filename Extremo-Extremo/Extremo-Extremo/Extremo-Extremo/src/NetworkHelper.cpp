...
NetworkHelper::StartServer(int port) {
  ...
  if (m_serverSocket == INVALID_SOCKET) {
    std::cerr << "Error creando el socket: " << WSAGetLastError() << std::endl;
    return false;
  }

  // Configurar la direccion del server (IPv4, cualquier local IP, puerto).
  sockaddr_in serverAddress{};
  serverAddress.sin_family = AF_INET;
  serverAddress.sin_port = htons(port);
  serverAddress.sin_addr.s_addr = INADDR_ANY;

  // Asociar el socket a la IP y al puerto.
  if (bind(m_serverSocket, (sockaddr*)&serverAddress, sizeof(serverAddress)) == SOCKET_ERROR) {
    std::cerr << "Error en el binding del socket: " << WSAGetLastError() << std::endl;
    closesocket(m_serverSocket);
...
NetworkHelper::AcceptClient() {
  ...
  if (clientSocket == INVALID_SOCKET) {
    std::cerr << "Error aceptando al cliente: " << WSAGetLastError() << std::endl;
...
NetworkHelper::ConnectToServer(const std::string& ip, int port) {
  ...
  if (connect(m_serverSocket, (sockaddr*)&serverAddress, sizeof(serverAddress)) == SOCKET_ERROR) {
    std::cerr << "Error conectando al server: " << WSAGetLastError() << std::endl;
    closesocket(m_serverSocket);
    m_serverSocket = INVALID_SOCKET;
    return false;
  }
  std::cout << "Conectado al server en " << ip << ":" << port << std::endl;
  return true;
}
...
NetworkHelper::ReceiveDataBinary(SOCKET socket, int size) {
  ...
  if (!ReceiveExact(socket, buffer.data(), size)) return {};
