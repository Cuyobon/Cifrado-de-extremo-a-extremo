...
CryptoHelper::GenerateRSAKeys() {
  BIGNUM* bn = BN_new(); // Crear BIGNUM para el exponente público.
  BN_set_word(bn, RSA_F4);
  rsaKeyPair = RSA_new();
  RSA_generate_key_ex(rsaKeyPair, 2048, bn, nullptr); // Generar clave RSA.
  BN_free(bn);
}
...
CryptoHelper::GetPublicKeyString() const {
  ...
  PEM_write_bio_RSAPublicKey(bio, rsaKeyPair); // Escribir la clave pública al BIO en formato PEM.
...
CryptoHelper::LoadPeerPublicKey(const std::string& pemkey) {
  BIO* bio = BIO_new_mem_buf(pemkey.data(), static_cast<int>(pemkey.size())); //Crear una BIO del string PEM.
  peerPublicKey = PEM_read_bio_RSAPublicKey(bio, nullptr, nullptr, nullptr); // Leer la clave publica de PEM.
  BIO_free(bio);
  if (!peerPublicKey) {
    throw std::runtime_error("Error al carcar la clave publica del Peer: "
...
CryptoHelper::EncryptAESKeyWithPeer() {
  if (!peerPublicKey) {
    throw std::runtime_error("Clave publica del Peer no cargada.");
  }
...
CryptoHelper::AESDecrypt(const std::vector<unsigned char>& ciphertext,
  ...
  if (EVP_DecryptFinal_ex(ctx, out.data() + outlen1, &outlen2) != 1) {
    EVP_CIPHER_CTX_free(ctx);
    return {}; // padding/key/iv incorrecto
  }
